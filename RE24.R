...
# RE24 w/RISP
data2019b %>%
  filter(!grepl("000", STATE) & !grepl("100", STATE)) -> data2019_risp
...
data2019_risp %>%  
  ...
  summarize(RE24 = sum(run_value),  
            PA = length(run_value),  
            Runs.Start = sum(Runs.State)) %>%
  mutate(Run.Pct = RE24/Runs.Start) -> runs_risp

runs_risp <- as.data.frame(runs_risp)
names(runs_risp)[2] <- "RE24_risp"
names(runs_risp)[3] <- "PA_risp"
names(runs_risp)[4] <- "Runs_Start_risp"



# RE24 w/o RISP
data2019b %>%
  filter(grepl("000", STATE) | grepl("100", STATE)) -> data2019_without_risp

data2019_without_risp %>%  
  group_by(BAT_ID) %>%  
  summarize(RE24 = sum(run_value),  
            PA = length(run_value),  
            Runs.Start = sum(Runs.State)) %>%
  mutate(Run.Pct = RE24/Runs.Start) -> runs_without_risp

runs_without_risp <- as.data.frame(runs_without_risp)
names(runs_without_risp)[2] <- "RE24_no_risp"
names(runs_without_risp)[3] <- "PA_no_risp"
names(runs_without_risp)[4] <- "Runs_Start_no_risp"


runs %>%
  inner_join(runs_risp, by = c("BAT_ID" = "BAT_ID")) %>%
  inner_join(runs_without_risp, by = c("BAT_ID" = "BAT_ID")) %>%
  select(-c("Run.Pct.x", "Run.Pct.y"))-> runs
